...
export function RestAgency() {
  ...
  const get = async () => {
    try {
      const response = await axios.get(url + "getResAgencyAdmin");
  ...
  const getClient = async () => {
    try {
      const response = await axios.get("http://localhost:4000/agency/getAgency");
  ...
  const peticionPost = async () => {
    ...
    try {
      await axios.post(url + "addResAgencyAdmin", { ...form, pDate: formattedDate });
  ...
  const selectRes = (reservation) => {
    ...
    setForm({
      ...
      pAgencyID: reservation.AgencyName,
      pDate: formattedDate,
    ...
    <>
      ...
      <div className="resAgency">
        ...
        <Modal isOpen={modalEliminar}>
          <ModalBody>
            Estás seguro que deseas eliminar la reservation {form && form.pAgencyID}
          </ModalBody>
          <ModalFooter>
